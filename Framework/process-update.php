...
session_start();
	...
	$QUERYGetFirstProcLoad = "SELECT id_product, product_code, name_product FROM product where f_customer = (select id_customer from customers where id_customer = $id_cust);";
$QUERYGetUserID="SELECT id_user as id_user FROM users WHERE user_email="."'".$_SESSION["Us3rN4mE"]."'".";";
$QueryProcessToEdit = "SELECT  CONCAT(u.user_name, ' ', u.user_last_name) as reporter,c.customer_name, ppc.name_process, pt.name_product,ppc.goal,ppc.id_process_product_customer,ppc.process_unit_cost,pt.product_code,c.id_customer
FROM process_product_customer AS ppc, customers AS c, product as pt,users as u
WHERE pt.f_customer = c.id_customer and ppc.f_product = pt.id_product and u.id_user = ppc.createdby and ppc.id_process_product_customer=$id;";
...
WHERE u.id_user = m.f_user_receiver and m.f_user_receiver = ".$_SESSION["Us3r1D"].";";	
	...
	$result_QueryProcessToEdit = mysqli_query($connection, $QueryProcessToEdit) or trigger_error("Query Failed! SQL: $QueryProcessToEdit - Error: ". mysqli_error($connection), E_USER_ERROR);
    ...
    $process = mysqli_fetch_assoc($result_QueryProcessToEdit);
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-info">
                            <div class="panel-heading">Actualizar Proceso</div>
                            <div class="panel-wrapper collapse in" aria-expanded="true">
                                <div class="panel-body">
                                    <form data-toggle="validator" action="insert_process.go" method="POST">
                                        <div class="form-body">
                                            <h3 class="box-title">Modifique la informacion que desee para actualizar el proceso.</h3>
                                            <hr>
                                            <div class="row">                                                
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="control-label">Nombre de Proceso</label>
                                                        <input type="text" class="form-control" required="" placeholder="Nombre con el que identifica al proceso" id="processname" name="processname" value="<?php echo $process["name_process"];?>" data-toggle="validator" data-error="Indique el nombre del proceso." required><div class="help-block with-errors"></div>
													</div>
                                                </div>
                                                <!--/span-->
                                               <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" step="any" min="0" class="form-control" required="" placeholder="0.00 Ingrese sin el signo de dolar" id="costo" name="costo" value="<?php echo $process["process_unit_cost"];?>" data-toggle="validator" data-error="Por favor ingresar el costo utilizando dos decimales." required><div class="help-block with-errors"></div> 
													</div>
                                                </div>
                                                <!--/span-->
                                            </div>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control" name="customerbox" id="customerbox" onchange="update(this.value,'procesos')">
                                                            ...
                                                            while($res1 = mysqli_fetch_assoc($result1)){
																if($process["id_customer"]==$res1["id_customer"]){
                                                    ...
                                                    <div class="form-group">
														...
														<?php
															...
															while($first_load_proc= mysqli_fetch_assoc($result3)){
																if($id_prod==$first_load_proc["id_product"]){
																 echo '<option selected="selected" value='.$first_load_proc["id_product"].'>'.$first_load_proc["product_code"].'  '.$first_load_proc["name_product"].'</option>';
																 }else{
																 echo '<option value='.$first_load_proc["id_product"].'>'.$first_load_proc["product_code"].'  '.$first_load_proc["name_product"].'</option>';	  //not working on selected.
                                                ...
                                                <!--/span-->
                                                 <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" id="goal" name="goal" value="<?php echo $process["goal"];?>" min="1" max="100000" class="form-control" required="" placeholder="Indique en las unidades de su preferencia (El valor debe ser un numero entero)." data-toggle="validator" data-error="Indique la meta para el proceso." required><div class="help-block with-errors"></div>
													</div>
                                                </div>
                                                <!--/span-->
                                            </div>
                                        ...
                                        <button type="submit" class="btn-danger btn-lg">Actualizar Proceso</button>
